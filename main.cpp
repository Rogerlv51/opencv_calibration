...
int main()
{
    ...
    {
        cout << "--> 第 " << ii + 1 << "图片的数据 --> : " << image_points_seq[ii][0].x << " -->" << image_points_seq[ii][0].y << endl;
    }
    cout << "角点提取完成！" << endl;

    //以下是摄像机标定
    cout << "开始标定………………";
    /*棋盘三维信息*/ // Size square_size = Size(26.5, 26.5);
    float square_size = 26.5f;  /* 实际测量得到的标定板上每个棋盘格的大小(角点间距，这个不能错，不然世界坐标有问题） */
